            ...
            try {
              ...
              for (let i of data.participants) {
                try {
                  ...
                  var pp_user = 'https://telegra.ph/file/929ce2bb1a20ed0774716.jpg'
                }
                if (data.action == "add") {
                  var but = [{buttonId: `/`, buttonText: { displayText: "\nGw mau jujur nih ke kalian, kalau gw sebenarnya gay😌" }, type: 1 }, {buttonId: `/`, buttonText: { displayText: "\nYey, akhirnya pacar gw masuk grup🤗" }, type: 1 }]
				conn.sendMessage(data.id, { caption: `Hallo👋🏻 @${i.split("@")[0]}\nSelamat Datang Di Grup *${metadata.subject}*\n\nIntro Dulu Yuk Kak\n\n\n-Nama : \n-Umur :\n-Askot :\n-Gender :\n\nSemoga Kamu Senang Berada Disini Serta Jangan Lupa Untuk Membaca Dan Mematuhi Rules Yang Ada`, image: {url: pp_user}, buttons: but, footer: `Deskripsi : ${metadata.desc}`, mentions: [i]})
		        } else if (data.action == "remove") {
		          var but = [{buttonId: `/`, buttonText: { displayText: "\nYah, kok pacar gw keluar😔" }, type: 1 }]
				conn.sendMessage(data.id, { caption: `Bye👋🏻 @${i.split("@")[0]}\n\n_Tetap Putus Asa Dan Jangan Semangat !_`, image: {url: pp_user}, buttons: but, footer: `${metadata.subject}`, mentions: [i]})
		}
              }
            } catch (e) {
              console.log(e)
