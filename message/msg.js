...
module.exports = async(conn, msg, m, setting, store, welcome) => {
	try {
		...
		const isOwner = ownerNumber == sender ? true : ["62895323263219@s.whatsapp.net", "6285752235008@s.whatsapp.net"].includes(sender) ? true : false
        ...
        async function sendPlay(from, query) {
           ...
           hxz.youtube(url).then(async(data) => {
             ...
             conn.sendMessage(from, { caption: `*Title :* ${data.title}\n*Quality :* ${data.quality}\n*Url :* https://youtu.be/${data.id}`, image: {url: data.thumb}, buttons: button, footer: 'Pilih Salah Satu Button Dibawahâ¬‡ï¸', mentions: [sender] } )
           }).catch((e) => {
		...
		const buttonsDefault = [
		    { urlButton: { displayText: `Instagram`, url : `${setting.youtubeOwner}` } },
		...
		const butMenu = [{buttonId: `${prefix}owner`, buttonText: { displayText: "Owner" }, type: 1 }, {buttonId: `${prefix}donasi`, buttonText: { displayText: "Donasi" }, type: 1 }]
		...
		if (isPlayGame(from, tebakgambar) && isUser) {
		  if (chats.toLowerCase() == getJawabanGame(from, tebakgambar)) {
			...
			var kus = [{buttonId: `${prefix}tebakgambar`, buttonText: { displayText: "Main Lagi" }, type: 1 }]
			 conn.sendMessage(from, { text: texttg, buttons: kus, footer: 'TEBAK GAMBAR', mentions: [sender]} )  
		...
		if (isPlayGame(from, caklontong) && isUser) {
		  if (chats.toLowerCase() == getJawabanGame(from, caklontong)) {
			...
			var kus = [{buttonId: `${prefix}caklontong`, buttonText: { displayText: "Main Lagi" }, type: 1 }]
			 conn.sendMessage(from, { text: texttg, buttons: kus, footer: 'CAK LONTONG', mentions: [sender]} )  
		...
		if (isPlayGame(from, ao) && isUser) {
		  if (chats.toLowerCase() == getJawabanGame(from, ao)) {
			...
			var kus = [{buttonId: `${prefix}asahotak`, buttonText: { displayText: "Main Lagi" }, type: 1 }]
			 conn.sendMessage(from, { text: texttg, buttons: kus, footer: 'ASAH OTAK', mentions: [sender]} )  
		...
		if (isPlayGame(from, tj) && isUser) {
		  if (chats.toLowerCase() == getJawabanGame(from, tj)) {
			...
			var kus = [{buttonId: `${prefix}tebakjenaka`, buttonText: { displayText: "Main Lagi" }, type: 1 }]
			 conn.sendMessage(from, { text: texttg, buttons: kus, footer: 'TEBAK JENAKA', mentions: [sender]} )  
...
case prefix+'menu': case prefix+'help': case prefix+'?':
			    ...
			    conn.sendMessage(from, { caption: teks, image: pp_bot, buttons: butMenu, footer: `${setting.footer}`, mentions: [sender] })
				break
			
case prefix+'runtime': case 'tes': case 'p': case 'bot':
			    reply(`Bot Sudah Aktif Selama:\n${runtime(process.uptime())}`)
			    break
...
var donasibut = [{buttonId: `${prefix}sewa`, buttonText: { displayText: "Sewa Bot" }, type: 1 }]
var teks = `──「 DONASI BOT 」──
...
_Terimakasih untuk kamu yang sudah donasi untuk perkembangan bot ini_`
 conn.sendMessage(from, { caption: teks, image: {url: `${setting.donasiQris}`}, buttons: donasibut, footer: 'DONASI UNTUK PERKEMBANGAN BOT', mentions: [sender]} )
			    break
			
case prefix+'sewa': case prefix+'sewabot':
var donasibut = [{buttonId: `${prefix}info`, buttonText: { displayText: "Info Lala Bot" }, type: 1 }]
var teks = `*Hai Kak ${pushname}*\n\nLIST SEWA\n- 1 MINGGU : 5k\n- 1 BULAN : 10K\n- PERMANEN : 25K\n\n\nJika Minat Hubungi Owner \n                   Wa.me/62895323263219 \n                                 Via GO-PAY / PULSA\n`
 conn.sendMessage(from, { caption: teks, image: {url: `${setting.donasiQris}`}, buttons: donasibut, footer: '© Lala BotzMD • by Zenz', mentions: [sender]} )
			    break

case prefix+'infobot': case prefix+'inforobot': case prefix+'info':
...
case prefix+'join':
        ...
        reply(`Sukses di Kirim Ke Owner, tunggu Laporan 3/4 menitan untuk di konfirmasi oleh Owner`)
...
case prefix+'hidetag': case prefix+'hide':
		        if (!isGroup) return reply(mess.OnlyGrup)
				if (!isGroupAdmins && !isOwner) return reply(mess.GrupAdmin)
...
case prefix+'infogc': case prefix+'infogrup': case prefix+'grupinfo': case prefix+'infogroup': case prefix+'groupinfo':
  if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
  if (!isGroup)return reply(mess.OnlyGrup)
...
case prefix+'promote': case prefix+'pm':
    ...
    if (mentioned.length !== 0) {
      ...
      .then( res => reply('_Sukses promote.._'))
...
case prefix+'demote': case prefix+'dm':
    ...
    if (mentioned.length !== 0) {
      ...
      .then( res => reply('_Sukses demote.._'))
      .catch( err => reply(jsonformat(err)))
    } else if (isQuotedMsg) {
      number = quotedMsg.sender
      conn.groupParticipantsUpdate(from, [number], "demote")
...
case prefix+'kick': case prefix+'k':
    if (!isGroup) return reply(mess.OnlyGrup)
    if (!isGroupAdmins && !isOwner) return reply(mess.GrupAdmin)
    if (!isBotGroupAdmins) return reply(mess.BotAdmin)
    var number;
